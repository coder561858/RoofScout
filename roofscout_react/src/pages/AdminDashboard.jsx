...
function AdminDashboard() {
  ...
  // CONTEXT FOR STATS
  const { getCombinedProperties, hiddenIds } = useContext(PropertyContext);
  const { getCombinedRentProperties, hiddenRentIds } = useContext(RentContext);

  const housesCount = useMemo(() => {
    const buyProps = getCombinedProperties({ mode: "buy" }) || [];
    ...
    return map.size;
  }, [getCombinedProperties, hiddenIds, getCombinedRentProperties, hiddenRentIds]);

  const plotsCount = useMemo(() => {
    const buyProps = getCombinedProperties({ mode: "buy" }) || [];
    const rentProps = getCombinedRentProperties({}) || [];

    // Filter out hidden, then filter to only "plots"
            ...
            {!collapsed ? (
              <>
                ...
                <div className="mt-2 grid grid-cols-1 gap-2">
                  <p>Houses Listed: <span className="text-white font-semibold">{housesCount}</span></p>
                  <p>Plots Listed: <span className="text-white font-semibold">{plotsCount}</span></p>
                </div>
              </>
            ) : (
